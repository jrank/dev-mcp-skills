...
- Using plain-text record type names in expressions — `recordType!Board Committee Submission.fields.status` will fail because the parser can't resolve names with spaces (or any names) without UUIDs; always use the UUID-qualified format `'recordType!{uuid}Name.fields.{fieldUuid}fieldName'` (with single quotes wrapping the entire reference)
